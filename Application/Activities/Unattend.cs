    ...
    public class Unattend
    {
        ...
        public class Handler : IRequestHandler<Command>
        {
            ...
            public async Task<Unit> Handle (Command request, CancellationToken cancellationToken)
            {
                ...
                //get user based on token, we're only here after already being authorized by server
                ...
                //if not attending, dont' do anything and exit
                if (attendance == null)
                {
                    return Unit.Value;
                }

                if (attendance.IsHost) throw new RestException(HttpStatusCode.BadRequest, new {Attendance = "You cannot remove yourself as a host"});
